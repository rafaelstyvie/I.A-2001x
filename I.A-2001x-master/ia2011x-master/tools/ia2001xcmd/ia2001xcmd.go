...
import (
	...
	"github.com/ia2001xpy-master/ia2001x/random"
	"github.com/ia2001xpy-master/ia2001x/secret"
)
...
func usage() {
	fmt.Printf(`
ia2001xcmd is a tool that uses authenticated encryption (Salsa20 with Poly1305) to encrypt/decrypt small files on disk.

Usage:
    ia2001xcmd command [flags]
...
func parseArguments(args []string) (mode string, keypath string, itercount int, input string, output string) {
	...
	err := fs.Parse(args[2:])
	if err != nil {
		fmt.Printf("ia2001xcmd: unable to parse flags: %v\n", err)
	}

	if mode != "encrypt" && mode != "decrypt" {
		fmt.Printf("ia2001xcmd: mode must be encrypt or decrypt, not: %q\n", mode)
	}
	if *in == "" {
		fmt.Printf("ia2001xcmd: input path required\n")
	}
	if *out == "" {
		fmt.Printf("ia2001xcmd: output path required\n")
	}
	if *in == "" || *out == "" {
		usage()
		os.Exit(255)
...
func encrypt(keypath string, itercount int, inputpath string, outputpath string) {
	...
	if err != nil {
		fmt.Printf("ia2001xcmd: unable to generate salt: %v\n", err)
		os.Exit(255)
	}

	key, isPass, err := generateKey(keypath, salt, itercount)
	if err != nil {
		fmt.Printf("ia2001xcmd: unable to generate or read in key: %v\n", err)
		os.Exit(255)
	}

	plaintextBytes, err := ioutil.ReadFile(inputpath)
	if err != nil {
		fmt.Printf("ia2001xcmd: unable to read plaintext file %q: %v\n", inputpath, err)
		os.Exit(255)
	}

	sealedData, err := secret.Seal(plaintextBytes, key)
	if err != nil {
		fmt.Printf("ia2001xcmd: unable to seal plaintext: %v\n", err)
		os.Exit(255)
	}

	err = writeCiphertext(salt, itercount, isPass, sealedData, outputpath)
	if err != nil {
		fmt.Printf("ia2001xcmd: unable to write sealed data to disk: %v\n", err)
		os.Exit(255)
	}
}
...
func decrypt(keypath string, inputpath string, outputpath string) {
	...
	if err != nil {
		fmt.Printf("ia2001xcmd: unable to read ciphertext file: %v\n", err)
		os.Exit(255)
	}

	key, _, err := generateKey(keypath, keySalt, keyIter)
	if err != nil {
		fmt.Printf("ia2001xcmd: unable to build key: %v\n", err)
		os.Exit(255)
	}

	plaintextBytes, err := secret.Open(sealedData, key)
	if err != nil {
		fmt.Printf("ia2001xcmd: unable to open ciphertext: %v\n", err)
		os.Exit(255)
	}

	err = ioutil.WriteFile(outputpath, plaintextBytes, 0600)
	if err != nil {
		fmt.Printf("ia2001xcmd: unable to write plaintext bytes to disk: %v\n", err)
		os.Exit(255)
	}
}
